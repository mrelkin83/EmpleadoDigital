...
### D34. Despliegue en VPS: dockerizado independiente, NO fusion con PAduanero (PENDIENTE de implementar)
Contexto: este proyecto es brazo de pedroabogadoaduanero.com (repo hermano `C:\laragon\www\PAduanero`), ya desplegado en su propio VPS. El usuario pregunto si fusionarlo con el sitio principal igual que se hizo con el motor de WhatsApp (`packages/whatsapp-engine` de PAduanero: paquete PHP puro sin dependencias, conectado via ~230 lineas de adaptadores porque ambos lados eran PHP/MySQL).

Hallazgo clave revisando PAduanero: el PO **retiro por completo** ese motor conversacional de WhatsApp + IA + pasarela de pagos en agosto 2026 (commit `3fcea6e`, -22.739 lineas) para volver a "una aplicacion PHP contra MySQL detras de Nginx, sin ningun proceso de fondo que vigilar" (`PAduanero/docs/RUNBOOK.md`). Las tablas de aquel sistema siguen vacias en la base por si acaso (ADR-013 de PAduanero), pero la direccion del PO es clara: menos superficie operativa, no mas.

Decision (a falta de implementar): NO fusionar codigo (empleado-digital es Node/TS/Fastify/Next/Postgres/worker; PAduanero es PHP/MySQL sin dependencias de produccion — no hay adaptador que los conecte sin reescribir uno de los dos). Estructura acordada:
- **PAduanero: sin cambios, sigue bare-metal** (PHP-FPM + Nginx + MySQL). Cero razon para dockerizarlo: no tiene dependencias que aislar y tocarlo es puro riesgo sin beneficio.
- **empleado-digital: dockerizado por completo** — contenedores `api`, `web`, `worker`, `postgres` en un `docker-compose.yml` de produccion, red y volumen propios. Mantiene Node/npm/Postgres fuera del host que PAduanero mantiene deliberadamente limpio.
- **Conexion**: el Nginx del VPS (el mismo bare-metal de siempre, sin dockerizar) agrega un `server_name` nuevo (ej. `empleado.pedroabogadoaduanero.com`) con reverse proxy al puerto publicado del contenedor — mismo patron que ya usan para separar `panel.pedroabogadoaduanero.com` de la landing en `PAduanero/docs/RUNBOOK.md` §4bis. Retirar el sistema despues (si algun dia se decide, como paso con WhatsApp) es `docker compose down` + borrar el bloque de Nginx, sin tocar PAduanero.
- Pendiente de resolver antes de implementar: mismo VPS que PAduanero o uno separado (el usuario no lo ha decidido); si es el mismo VPS, confirmar recursos disponibles (RAM/CPU) para correr Postgres + los tres servicios Node junto al PHP-FPM existente.

Estado: solo documentado, sin Dockerfiles ni compose de produccion escritos todavia. Retomar cuando el usuario lo pida.
